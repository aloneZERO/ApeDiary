...
<div class="data_list">
		...
		<c:choose>
			<c:when test="${category.id!=null}">
				<img src="${pageContext.request.contextPath}/web/images/category_edit_icon.png"/>
				修改日记类别
			</c:when>
			<c:otherwise>
				<img src="${pageContext.request.contextPath}/web/images/category_add_icon.png"/>
				添加日记类别
			</c:otherwise>
		</c:choose>
		</div>
		<form action="${pageContext.request.contextPath}/client/category?action=save"
